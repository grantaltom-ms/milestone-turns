...
export async function loadTaskNotes(turnId: string, stageIdx?: number): Promise<TaskNote[]> {
  ...
  let q = supabase
    .from("task_notes")
    .select("id, turn_id, stage_idx, task_name, author_id, content, photo_urls, created_at, profiles(name)")
    .eq("turn_id", turnId);
  ...
  return ((data ?? []) as unknown[]).map((row) => {
    const r = row as {
      ...
      author_id: string; content: string | null; photo_urls: string[]; created_at: string;
    ...
    return {
      ...
      content: r.content, photo_urls: r.photo_urls ?? [], created_at: r.created_at,
    } satisfies TaskNote;
  });
}
