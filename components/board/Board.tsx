...
type Filter = "All" | "Office" | "Maintenance" | "Ready" | "Mine" | "On Hold";
const FILTERS: Filter[] = ["All", "Mine", "Office", "Maintenance", "Ready", "On Hold"];
            ...
            return (
              <button key={f} type="button" onClick={() => setFilter(f)}
                style={{
                  padding: "6px 14px",
                  borderRadius: 999,
                  border: `1.5px solid ${active ? "transparent" : isHoldChip ? "rgba(200,146,42,0.5)" : "rgba(245,241,232,0.3)"}`,
                  background: active ? (isHoldChip ? "#C8922A" : "#F5F1E8") : "transparent",
                  color: active ? (isHoldChip ? "#fff" : "#1A2E44") : isHoldChip ? "#C8922A" : "rgba(245,241,232,0.8)",
              ...
              >
                {f}
                {isHoldChip && onHoldCount > 0 && (
                  <span style={{ background: active ? "rgba(255,255,255,0.3)" : "rgba(200,146,42,0.18)", color: active ? "#fff" : "#C8922A", borderRadius: 999, padding: "1px 6px", fontSize: 11, fontWeight: 700, lineHeight: 1.4 }}>
                    {onHoldCount}
                  </span>
                )}
              </button>
